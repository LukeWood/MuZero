...
def update_weights(optimizer: tf.keras.optimizers, network: BaseNetwork, batch):
    ...
    def loss():
        ...
        target_policy_batch = list(
            filter(lambda l: bool(l), target_policy_batch))
        policy_batch = tf.boolean_mask(policy_batch, mask_policy)

        # Compute the loss of the first pass
        loss += tf.math.reduce_mean(loss_value(target_value_batch,
                                               value_batch, network.value_support_size))
        ...
        for actions_batch, targets_batch, mask, dynamic_mask in zip(actions_time_batch, targets_time_batch,
            ...
            representation_batch = tf.boolean_mask(
                representation_batch, dynamic_mask)
            ...
            conditioned_representation_batch = tf.concat(
                (representation_batch, actions_batch), axis=1)
            ...
            target_policy_batch = [policy for policy,
                                   b in zip(target_policy_batch, mask) if b]
            mask_policy = list(map(lambda l: bool(l), target_policy_batch))
            target_policy_batch = tf.convert_to_tensor(
                [policy for policy in target_policy_batch if policy])
            policy_batch = tf.boolean_mask(policy_batch, mask_policy)
